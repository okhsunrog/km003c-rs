...
fn fs_main(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    for (var ch = 0u; ch < params.num_channels; ch++) {
        ...
        // The glow is a faint tint of the line colour. A darkened glow read as
        // a grey shadow on light backgrounds.
        let glow_alpha = smoothstep(px_y * 6.0 * params.scale, 0.0, dist) * 0.15;

        color = mix(color, line_col, glow_alpha);
...
fn fs_line(input: LineVertex) -> @location(0) vec4<f32> {
    ...
    // A faint tint of the line colour; see the envelope pass above.
    let glow = (1.0 - smoothstep(0.0, 4.0 * params.scale, distance)) * 0.12;
