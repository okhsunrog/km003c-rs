...
impl PlotBuffer {
    ...
    pub fn read_back(&self, frames_back: usize, dst: &mut [f32]) {
        ...
        let base = ((inner.written - frames_back as u64) % self.capacity as u64) as usize * self.num_channels;
    ...
    pub(crate) fn sync_to(&self, dst: &mut Vec<f32>, snapshot: &mut Snapshot) -> Vec<Range<usize>> {
        let inner = self.inner.lock().unwrap();
        if snapshot.source == self.id && snapshot.initialized && snapshot.generation == inner.generation {
            return Vec::new();
        }
