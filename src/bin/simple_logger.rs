...
const AUTH_PAYLOAD_1: &[u8] = &[0x33, 0xf8, 0x86, 0x0c, 0x00, 0x54, 0x28, 0x8c, 0xdc, 0x7e, 0x52, 0x72, 0x98, 0x26, 0x87, 0x2d, 0xd1, 0x8b, 0x53, 0x9a, 0x39, 0xc4, 0x07, 0xd5, 0xc0, 0x63, 0xd9, 0x11, 0x02, 0xe3, 0x6a, 0x9e];
const AUTH_PAYLOAD_2: &[u8] = &[0x63, 0x6b, 0xea, 0xf3, 0xf0, 0x85, 0x65, 0x06, 0xee, 0xe9, 0xa2, 0x7e, 0x89, 0x72, 0x2d, 0xcf, 0xd1, 0x8b, 0x53, 0x9a, 0x39, 0xc4, 0x07, 0xd5, 0xc0, 0x63, 0xd9, 0x11, 0x02, 0xe3, 0x6a, 0x9e];
const AUTH_PAYLOAD_3: &[u8] = &[0xc5, 0x11, 0x67, 0xae, 0x61, 0x3a, 0x6d, 0x46, 0xec, 0x84, 0xa6, 0xbd, 0xe8, 0xbd, 0x46, 0x2a, 0xd1, 0x8b, 0x53, 0x9a, 0x39, 0xc4, 0x07, 0xd5, 0xc0, 0x63, 0xd9, 0x11, 0x02, 0xe3, 0x6a, 0x9e];
const AUTH_PAYLOAD_4: &[u8] = &[0x9c, 0x40, 0x9d, 0xeb, 0xc8, 0xdf, 0x53, 0xb8, 0x3b, 0x06, 0x6c, 0x31, 0x52, 0x50, 0xd0, 0x5c, 0xd1, 0x8b, 0x53, 0x9a, 0x39, 0xc4, 0x07, 0xd5, 0xc0, 0x63, 0xd9, 0x11, 0x02, 0xe3, 0x6a, 0x9e];
const SET_RECORDER_MODE_PAYLOAD: &[u8] = &[0x4b, 0xe3, 0x63, 0x6c, 0x40, 0xbc, 0x10, 0x29, 0x89, 0x50, 0x96, 0xaa, 0xa3, 0xd2, 0x4f, 0xb7, 0xf0, 0x9b, 0x3f, 0xfb, 0x91, 0xb6, 0x51, 0xf1, 0x58, 0x2d, 0x0c, 0x27, 0xe4, 0x8d, 0x43, 0xa2];
...
async fn run(cli: Cli) -> Result<()> {
    ...
    for i in 0..iterations {
        comms.send_command(CommandType::GetData, Attribute::AdcQueue, None).await?;
...
async fn perform_startup_sequence(comms: &mut DeviceComms) -> Result<()> {
    ...
    comms.send_command(CommandType::Authenticate, Attribute::from_u16(0x0101), Some(AUTH_PAYLOAD_1)).await?;
    comms.read_and_discard().await?; comms.read_and_discard().await?;
    comms.send_command(CommandType::Authenticate, Attribute::from_u16(0x0101), Some(AUTH_PAYLOAD_2)).await?;
    comms.read_and_discard().await?; comms.read_and_discard().await?;
    comms.send_command(CommandType::Authenticate, Attribute::from_u16(0x0101), Some(AUTH_PAYLOAD_3)).await?;
    comms.read_and_discard().await?; comms.read_and_discard().await?;
    comms.send_command(CommandType::Authenticate, Attribute::from_u16(0x0101), Some(AUTH_PAYLOAD_4)).await?;
    comms.read_and_discard().await?; comms.read_and_discard().await?;
    info!("--- Authentication Replay Complete ---");

    info!("--- Setting Recorder Mode ---");
    comms.send_command(CommandType::SetRecorderMode, Attribute::from_u16(0x0200), Some(SET_RECORDER_MODE_PAYLOAD)).await?;
    comms.read_and_discard().await?;

    info!("--- Initial Info Dump ---");
    comms.send_command(CommandType::GetData, Attribute::PdPacket, None).await?;
    comms.read_and_discard().await?;
    comms.send_command(CommandType::GetData, Attribute::from_u16(0x0400), None).await?;
    comms.read_and_discard().await?;
    
    info!("--- Stopping Stream for Clean State ---");
    comms.send_command(CommandType::StopStream, Attribute::None, None).await?;
    comms.expect_response(CommandType::Accept).await?;
...
impl DeviceComms {
    ...
    async fn expect_response(&mut self, expected_cmd: CommandType) -> Result<Bytes> {
        ...
        if response[1] != self.transaction_id {
            warn!("Mismatched transaction ID. Expected {}, got {}", self.transaction_id, response[1]);
        }
